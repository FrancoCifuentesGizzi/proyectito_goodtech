...
class detalle:
    ...
    def llenar_treeview_detalle(self):
        sql = """select producto_id_producto, venta_id_venta, precio_venta, cantidad from detalle_venta
            join producto on detalle_venta.producto_id_producto = producto.id_producto
            join venta on detalle_venta.venta_id_venta = venta.id_venta order by id_venta asc where venta_id_venta = %(id_venta)s"""
        data = self.db.run_select_filter ( sql, {"id_venta": int(self.row_data[0])} )
    ...
    def __Agregar_PV(self):
        Add_ProVent(self.db, self, row_data)
...
class Add_ProVent:
    ...
    def __config_labels(self):
        ...
        tk.Label(self.add ,text = "Precio: ").place(x = 0, y = 60, width = 100, height = 20)
        tk.Label(self.add ,text = "Cantidad: ").place(x = 0, y = 85, width = 100, height = 20)

    #Configuración de las casillas que el usuario ingresa info
    def __config_entry(self):
        self.comboproducto = ttk.Combobox(self.add)
        self.comboproducto.place(x = 100, y = 35, width = 100, height = 20)
        self.comboproducto["values"], self.pro = self.__fill_combo_pro ()
        self.entry_precio = ttk.Entry(self.add)
        self.entry_precio.place(x = 100, y = 10, width = 100, height = 20)
        self.entry_cantidad = ttk.Entry(self.add)
        self.entry_cantidad.place(x = 100, y = 10, width = 100, height = 20)
    ...
    def __insertar(self):  # Insercion en la base de datos.
        ...
        sql = """call ProductoIn(%(cantidad)s, $(precio)s, $(id_venta)s);"""
