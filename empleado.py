...
class empleado:
    ...
    def __Eliminar_E(self):
        ...
        row_data = self.db.run_select_filter ( sql, {"id_empleado": self.treeview.focus ()} )[0]
        Eliminar_Empleado( self.db, self, row_data )
...
class editar_empleado:  # Clase para modificar
    ...
    def __config_entry(self):
        ...
        self.combosuc["values"], self.suc = self.fill_combosuc ()
        self.comboper = ttk.Combobox(self.insert_datos)
        self.comboper.place(x = 100, y = 160, width = 120, height = 20)
        self.comboper["values"], self.per = self.fill_comboper ()
    ...
    def modificar(self):  # Insercion en la base de datos.
        ...
        self.db.run_sql ( sql, {"nombre_emp": self.entry_nombre.get (),
                                "apellido_emp": self.entry_apellido.get (),
                                "telefono_emp": self.entry_telefono.get (),
                                "id_sucursal": self.suc[self.combosuc.current ()],
                                "id_perfil": self.per[self.comboper.current ()],
...
class Eliminar_Empleado:
    ...
    def __config_label(self):
        ...
        tk.Label ( self.del_datos, text= "Apellido: " + (self.row_data[1])).place( x=5, y=50, width=250, height=20 )
        # tk.Label ( self.del_datos, text= "Sucursal: " + (self.row_data[1])).place( x=5, y=70, width=250, height=20 ) # el id es int
